...
export const AffiliationForm = ({ onNext, onBack }: Props) => {
    const { t } = useTranslation();
    const { updateFormData, formData } = useFeedback();
    const [selected, setSelected] = useState(formData.affiliation || "");
                ...
                <div className="block">
                    ...
                    <div className="space-y-3 mt-2">
                        <label className="flex items-center gap-3 cursor-pointer">
                            <Input type="radio" name="affiliation" value="General Public" checked={selected === "General Public"} onChange={(e) => setSelected(e.target.value)} className="h-4 w-4" />
                            <span className="text-[var(--theme-text)]">{t("feedback.affiliation.generalPublic")}</span>
                        </label>
                        <label className="flex items-center gap-3 cursor-pointer">
                            <Input type="radio" name="affiliation" value="Government (Within LGU)" checked={selected === "Government (Within LGU)"} onChange={(e) => setSelected(e.target.value)} className="h-4 w-4" />
                            <span className="text-[var(--theme-text)]">{t("feedback.affiliation.withinLgu")}</span>
                        </label>
                        <label className="flex items-center gap-3 cursor-pointer">
                            <Input type="radio" name="affiliation" value="Government (Other Offices/Agencies)" checked={selected === "Government (Other Offices/Agencies)"} onChange={(e) => setSelected(e.target.value)} className="h-4 w-4" />
